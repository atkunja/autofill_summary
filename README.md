...
- Ashby nested education labels, split month/year menus, canonical school search results, and Yes/No answer buttons. The profile includes an explicit current-student choice for the education checkbox. Missing dates and unavailable dropdown answers remain for review; exact graduation days are never invented.
- Site-independent ARIA button dropdowns and radio groups, cautiously associated nearby labels, and open shadow-root controls. Existing selections and disabled controls are preserved.
- Embedded forms in frames Chrome permits the extension to access. Each preview is bound to its document; navigating a frame invalidates its preview. Inaccessible frames are reported so you can open the application directly. No new host permissions are requested by scanning.
...
**Compatibility is best effort, not universal.** Frames without permission, closed shadow roots, unsupported custom widgets, hidden upload inputs, and unusual ATS controls may need manual entry. Greenhouse React Select behavior is covered with an actual React Select integration fixture. Ashby controls are tested with a React fixture modeled on the Ramp application, including school selection, sponsorship, and preservation of existing answers. This is not a claim that every Greenhouse, Ashby, or Workday form is supported. Shared controls, embedded forms, navigation, and multi-step rescanning have browser regression tests. Workday, Lever, iCIMS, and other ATS products are not yet verified end to end on live applications. Scan each new step of a multi-step form. The extension neither checks consent boxes nor submits applications. A website may immediately upload or autosave a value after you fill it; review the destination before filling.
...
New PDF/TXT uploads prepare resume text and recognized education when experience text is empty; review and Save profile to apply. The application panel now counts filled, selected, and attention-needed detected fields. Unmatched ordinary questions now support saved profile mappings. Form changes offer a rescan; edited drafts survive when the question and document match. Filled answers are checked after page updates. Batch drafting requires company/role context, runs two requests at a time (up to eight fields), supports cancellation, and keeps answers unselected until review. See [reliability behavior and limits](docs/AUTOFILL-IMPROVEMENTS.md) and the [ATS verification checklist](docs/ATS-LIVE-CHECKLIST.md). Fixture success does not establish live ATS compatibility.
