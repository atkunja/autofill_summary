...
test('sensitive and ambiguous fields stay manual',()=>{
  for(const label of ['Visa sponsorship','Gender','Salary expectations','Social security number','Citizenship','Disability','Work authorization']){assert.equal(classify({label}),null);assert.equal(canDraft({label,tag:'textarea'}),false);}
